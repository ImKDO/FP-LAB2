...
let string_hash s =
  let hash = ref 0 in
  String.iter (fun c -> 
    hash := (!hash * 31 + Char.code c) mod max_int
  ) s;
  !hash

let test_string_keys () =
  let m = 
    H.empty ~hash:string_hash 
    |> H.add "apple" 1
    |> H.add "banana" 2
    |> H.add "cherry" 3
  in
...
let test_immutability () =
  ...
  Alcotest.(check (option int)) "original unchanged value" (Some 10) (H.find_opt 1 m1);
