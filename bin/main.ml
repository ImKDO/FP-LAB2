...
let print_map_entry k v = Printf.printf "  %s -> %d\n" k v
...
let () =
  ...
  let map =
    empty_map |> Hashmap.add "one" 1 |> Hashmap.add "two" 2
    |> Hashmap.add "three" 3
  in
  Printf.printf "   Size: %d\n" (Hashmap.size map);
  ...
  let map2 =
    Hashmap.empty ~hash:string_hash
    |> Hashmap.add "four" 4 |> Hashmap.add "five" 5
  in
