...
let resize t new_cap =
  ...
  let (final_buckets, final_size) =
    List.fold_left (fun (buckets, n) (k, v) ->
...
let empty ~hash = {
  hash;
  size = 0;
  buckets = Array.make default_capacity Empty;
}
...
let fold_left f acc t =
  Array.fold_left (fun current_acc entry ->
    match entry with
    | Entry (k, v) -> f current_acc k v
    | _ -> current_acc
  ) acc t.buckets

let to_list t = 
  fold_left (fun acc k v -> (k, v) :: acc) [] t
